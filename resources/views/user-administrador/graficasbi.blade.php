...
<script>
    ...
    const noResultElement = document.getElementById(`no-result-${chart.form_id}`);

    if (!searchTerm) {
        // Restaurar el estado original de la tabla y gráfica si el filtro está vacío
        filas.forEach(fila => {
            fila.style.display = ''; // Mostrar todas las filas
        });
        noResultElement.style.display = 'none'; // Ocultar mensaje de "Resultado no encontrado"
        
        // Restaurar datos originales a la gráfica
        chartInstance.data.labels = chart.labels;
        chartInstance.data.datasets[0].data = chart.data;
    ...
    } else if (mesesEspanol.includes(searchTerm.toLowerCase())) {
        filas.forEach(fila => {
            const mesTexto = fila.querySelector('td:first-child').textContent.toLowerCase();
            if (mesTexto === searchTerm.toLowerCase()) {
                fila.style.display = ''; // Mostrar fila
                filteredData.push(parseInt(fila.children[1].textContent));
                filteredLabels.push(mesTexto);
                found = true;
    ...
    } else if (esFechaValida(searchTerm)) {
        const mesDeFecha = obtenerMesDeFecha(searchTerm);
        filas.forEach(fila => {
            const mesTexto = fila.querySelector('td:first-child').textContent.toLowerCase();
            if (mesTexto === mesDeFecha) {
                fila.style.display = ''; // Mostrar fila
                filteredData.push(parseInt(fila.children[1].textContent));
                filteredLabels.push(mesTexto);
                found = true;
    ...
    if (found) {
        ...
        mostrarError(filas, chart.form_id);
            ...
            // Obtener el mes de una fecha en formato ddmmyy o ddmmyyyy
            function obtenerMesDeFecha(fecha) {
                const regexFecha = /^(\d{2})(\d{2})(\d{2,4})$/;
                const match = fecha.match(regexFecha);
                if (match) {
                    const mes = parseInt(match[2]) - 1; // El mes es el segundo grupo
                    const mesesEspanol = [
                        'enero', 'febrero', 'marzo', 'abril', 'mayo', 'junio',
                        'julio', 'agosto', 'septiembre', 'octubre', 'noviembre', 'diciembre'
                    ];
                    return mesesEspanol[mes];
                }
                return '';
            }
