    ...
    public function dataTable($query)
    {
        ...
        if($this->form_id == 18){
            ...
            ->escapeColumns('active')
                ...
                ->editColumn('status', function (FormValue $formValue) {
                    if ($formValue->status == "Verificado") {
                        $out = '<span class="p-2 px-3 badge rounded-pill bg-primary">' . __('Verificado') . '</span>';
                        return $out;
                    } else if ($formValue->status == "Pendiente por modificar") {
                        $out = '<span class="p-2 px-3 badge rounded-pill bg-warning">' . __('Pendiente por modificar') . '</span>';
                        return $out;
                    } else if ($formValue->status == "Por autenticar") {
                        $out = '<span class="p-2 px-3 badge rounded-pill bg-success">' . __('Por autenticar') . '</span>';
                        return $out;
                    } else {
                        $out = '<span class="p-2 px-3 badge rounded-pill bg-danger">' . __('No autorizado') . '</span>';
                        return $out;
                    }
                })
                ...
                ->editColumn('user', function (FormValue $formValue) {
                    ...
                    $nombre = array_filter($arr[0], function($campo) {
                      ...
                      if($campo['label'] == "Nombre completo" || $campo['label'] == "Nombre" || $campo['label'] == "Nombre de la persona que utilizó el DEA" || $campo['label'] == "Nombre Completo" || $campo['label'] == "Nombre del lugar" || $campo['label']  == "Tí­tulo del Acta"){
