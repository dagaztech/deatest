...
/*
    ...
    public function generateDynamicChartsAndTables($id)
    {
        // Obtener los valores del formulario

        if (!$id) {
            abort(400, 'El ID del formulario es requerido para generar las gráficas.');
        }
        ...
        foreach ($formularios as $value) {
            ...
            // Procesar relaciones entre formularios 13 y 14 si aplican
            if ($formValue->form_id == 13) {
                $arr = json_decode($formValue->json, true);
                $identificacionCampo = array_filter($arr[0], function ($campo) {
                    return $campo['label'] == "Número de documento de identificación del paciente";
                });
            ...
            if ($formValue->form_id == 14) {
                $arr = json_decode($formValue->json, true);
                $identificacionCampo = array_filter($arr[0], function ($campo) {
                    return $campo['label'] == "Número de documento de identidad";
                });
            ...
            // Añadir datos procesados al resultado
            array_push($resultado, $array);
        }
    
        // Crear estructuras dinámicas para tablas y gráficos
        $columns = [];
        $tableData = [];
    
        foreach ($resultado as $row) {
            foreach ($row as $field) {
                $columns[] = $field->label; // Agregar columnas dinámicamente
                $tableData[] = [
                    'label' => $field->label,
                    'value' => $field->value
                ];
        ...
        // Evitar duplicados en los nombres de las columnas
        $columns = array_unique($columns);
    
        // Enviar datos a la vista
        return view('user-administrador.graficasbi', compact('columns', 'tableData', 'resultado'));
    }
