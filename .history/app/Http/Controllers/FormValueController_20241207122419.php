...
class FormValueController extends Controller
{
    ...
    public function generateCharts()
    {
       ...
       /* foreach ($forms as $form) {
        ...
        foreach ($forms as $form) {
            //$columns = ['Titulo del plan', 'DEA en revisión', 'Descripción del plan de mejora']; // Reemplaza con datos reales
            $data = array_map(function () {
                return rand(10, 100); // Generar valores aleatorios para cada mes
            }, range(1, count($months)));
            ...
            $charts[] = [
                'form_id' => $form->id,
                'type' => $chartTypes[array_rand($chartTypes)], // Tipo de gráfico aleatorio
                'labels' => $months,
                'data' => $data,
                //'columns' => $columns, // Pasar las columnas al gráfico
            ];
        }
