...
public function graphSubmitedForms($form_id, FormValuesDataTable $dataTable)
{
    // Obtén todos los formularios
    $forms = Form::all();
    
    // Obtén los datos para el gráfico (esto depende de la lógica de tu aplicación)
    // Aquí, asumimos que `UtilityFacades::dataChart($form_id)` ya devuelve los datos correctos para el gráfico
    $chartData = UtilityFacades::dataChart($form_id); 

    // Esto puede ser un ejemplo de estructura de datos que necesitas para Chart.js
    $labels = $chartData['labels'];  // Etiquetas para el gráfico (e.g., nombres de las categorías)
    $data = $chartData['data'];      // Datos para el gráfico (e.g., valores de cada categoría)

    // También puedes obtener los detalles del formulario
    $formsDetails = Form::find($form_id);

    // Pasamos los datos del gráfico junto con los otros datos a la vista
    return $dataTable->with('form_id', $form_id)->render('user-administrador.graficasbi', compact('forms', 'formsDetails', 'labels', 'data'));
}
