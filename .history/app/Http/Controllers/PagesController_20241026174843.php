...
/*
    ...
    public function tablerosuseradmin(Request $request)
    {
        ...
        if ($request->ajax() && $request->has('rango')) {
            $rango = explode(" to ", $request->rango);
            $start = Carbon::parse($rango[0]);
            $end = Carbon::parse($rango[1]);
    
            // Obtener respuestas para uso del DEA
            $respuestas = FormValue::where("form_id", 14)->whereBetween('created_at', [$start, $end])->get();
            foreach ($respuestas as $value) {
                $lista[] = date("m", strtotime($value->updated_at));
            }
    
            // Obtener respuestas para edad promedio
            foreach ($respuestas as $value) {
                $arr = json_decode($value->json, true);
                $edad = collect($arr[0])->firstWhere('label', 'Edad');
                $lista3[] = $edad ? $edad['value'] : null;
            }
    
            // Obtener respuestas para instalación del DEA
            $respuestas2 = FormValue::where("form_id", 11)->whereBetween('created_at', [$start, $end])->get();
            foreach ($respuestas2 as $value) {
                $arr = json_decode($value->json, true);
            ...
            return response()->json(['lista' => $lista, 'lista2' => $lista2, 'lista3' => array_count_values($lista3)]);
        }
    
        // Si no es AJAX, cargar la vista sin datos
        return view('user-administrador.tableros-useradmin');
    }
