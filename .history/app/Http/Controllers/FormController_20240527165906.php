...
*/
    ...
    public function eventos_criticosactivos()
    {
        // Obtener los datos del form ID=12 ordenados por fecha
       // $datos = FormValue::where('form_id', 12)->orderBy('created_at', 'desc')->get(['id', 'Coordenada: Longitud', 'Coordenada: Latitud', 'created_at']);

       // return view('user-consultassm.eventos-criticosactivos', compact('datos'));


               
            $puntoInstalacionDea = FormValue::where("form_id", 12)
                                                    ->where("json", 'LIKE', '%'.'value":"'.strtolower($arr2[0]).'"}'.'%', )
                                                   // ->where("json", 'LIKE', '%'.'"value":"'.$arr2[1].'"}'.'%', )
                                                    ->first();
    
            print_r($puntoInstalacionDea);
    
    
            $respuestasPinstalacion = json_decode($puntoInstalacionDea->json,true);
            
    
            $fechaInstalacionObjeto = array_filter($respuestasPinstalacion[1], function($campo) {
              if((str_contains(strtoupper($campo['label']), strtoupper("Fecha de registro del DEA")))){
    ...
    public function informe_ubicaciondeas()
    {
        // Obtener los datos del form ID=33 ordenados por fecha
        $datos = FormValue::where('form_id', 33)->orderBy('created_at', 'desc')->get(['id', 'Coordenada: Longitud', 'Coordenada: Latitud', 'created_at']);
