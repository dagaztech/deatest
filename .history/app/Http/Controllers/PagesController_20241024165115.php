...
class PagesController extends Controller
{
    ...
    public function tablerosuseradmin(Request $request) {
        // Consulta para la primera gráfica
        $respuestas = FormValue::where("form_id", 14);
    
        // Filtro de búsqueda
        if (isset($request->busqueda)) {
            $respuestas = $respuestas->where('tbl_form_values.json', 'iLIKE', '%' . strtolower($request->busqueda) . '%');
        }
        ...
        if (isset($request->rango)) {
            ...
            if (count($rango) == 2) {
                $start = $rango[0];
                $end = $rango[1];
                $respuestas = $respuestas->whereBetween('created_at', [Carbon::parse($start), Carbon::parse($end)]);
            } elseif (count($rango) == 1) {
                $fecha = $rango[0];
                $respuestas = $respuestas->whereDate('created_at', Carbon::parse($fecha));
            }
        }
        ...
        foreach ($respuestas as $value) {
            array_push($lista, ['mes' => date("m", strtotime($value->updated_at))]);
        }
    
        // Datos para la tercera gráfica (edades)
        $lista3 = array();
        foreach ($respuestas as $value) {
            ...
            array_push($lista3, ["edad" => $nombre[6]["value"]]);
        }
    
        // Consulta para la segunda gráfica
        $respuestas2 = FormValue::where("form_id", 11);
    
        if (isset($request->busqueda)) {
            $respuestas2 = $respuestas2->where('tbl_form_values.json', 'iLIKE', '%' . strtolower($request->busqueda) . '%');
        }
        ...
        if (isset($request->rango)) {
            ...
            if (count($rango) == 2) {
                $start = $rango[0];
                $end = $rango[1];
                $respuestas2 = $respuestas2->whereBetween('created_at', [Carbon::parse($start), Carbon::parse($end)]);
            } elseif (count($rango) == 1) {
                $fecha = $rango[0];
                $respuestas2 = $respuestas2->whereDate('created_at', Carbon::parse($fecha));
            }
        }
        ...
        // Retorno de las listas a la vista
