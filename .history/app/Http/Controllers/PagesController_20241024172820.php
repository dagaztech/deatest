...
class PagesController extends Controller
{
    ...
    public function tablerosuseradmin(Request $request)
    {
        ...
        // Obtener todas las respuestas del form 14
        $respuestas = FormValue::where('form_id', $formId1);
    
        // Filtro por búsqueda de texto
        if ($request->has('busqueda')) {
            $busqueda = strtolower($request->busqueda);
            $respuestas->whereRaw("LOWER(json) ILIKE ?", ["%{$busqueda}%"]);
        }
        ...
        if ($request->has('rango')) {
            ...
            // Si el rango incluye dos fechas
            if (count($rango) == 2) {
                $start = Carbon::parse($rango[0]);
                $end = Carbon::parse($rango[1]);
                $respuestas->whereBetween('created_at', [$start, $end]);
            } else {
                // Si solo es una fecha
                $fecha = Carbon::parse($rango[0]);
        ...
        // Procesar lista de meses
        $lista = $respuestas->map(function ($item) {
            return ['mes' => date("m", strtotime($item->updated_at))];
        })->toArray();
    
        // Procesar lista de edades
        $lista3 = $respuestas->map(function ($item) {
            $arr = json_decode($item->json, true);
            $edad = collect($arr[0])->firstWhere('label', 'Edad');
            return ['edad' => $edad['value']];
        })->toArray();
    
        // Obtener todas las respuestas del form 11
        $respuestas2 = FormValue::where('form_id', $formId2);
    
        // Aplicar los mismos filtros para respuestas2
        if ($request->has('busqueda')) {
            $respuestas2->whereRaw("LOWER(json) ILIKE ?", ["%{$busqueda}%"]);
        }
        ...
        if ($request->has('rango')) {
            if (count($rango) == 2) {
                $start = Carbon::parse($rango[0]);
                $end = Carbon::parse($rango[1]);
                $respuestas2->whereBetween('created_at', [$start, $end]);
            } else {
                $fecha = Carbon::parse($rango[0]);
        ...
        // Procesar lista2 para "Cantidad de DEAs"
        $lista2 = $respuestas2->map(function ($item) {
            $arr = json_decode($item->json, true);
            $cantidad = collect($arr[0])->firstWhere('label', 'Cantidad de DEAs');
            return [
                'mes' => date("m", strtotime($item->updated_at)),
