...
/*
    ...
    public function generateCharts()
{
    // Obtener todos los formularios desde la tabla tbl_forms
    $forms = Form::all();

    // Inicializar estructura para gráficos
    $charts = [];
    $months = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'];

    // Iterar sobre cada formulario
    foreach ($forms as $form) {
        // Obtener las columnas dinámicas de la tabla tbl_form_fields
        $columns = \DB::table('tbl_form_fields')
            ->where('form_id', $form->id)
            ->pluck('field_name')
            ->toArray();

        // Inicializar datos para cada columna
        $columnData = [];
        foreach ($columns as $column) {
            // Obtener registros agrupados por mes y columna desde tbl_form_data
            $data = \DB::table('tbl_form_data')
                ->selectRaw('MONTH(created_at) as mes, COUNT(*) as total')
                ->where('form_id', $form->id)
                ->whereNotNull($column) // Asegurar que la columna tenga datos
                ->groupBy('mes')
                ->orderBy('mes')
                ->pluck('total', 'mes')
                ->toArray();
            ...
            $monthlyData = array_map(function ($mes) use ($data) {
                return $data[$mes] ?? 0; // Si no hay datos, usar 0
            }, range(1, 12));

            // Guardar datos de la columna
            $columnData[$column] = $monthlyData;
        }

        // Agregar datos del formulario al array de gráficos
        $charts[] = [
            'form_id' => $form->id,
            'form_name' => $form->name,
            'columns' => $columns,
    ...
    // Retornar la vista con los datos generados para los gráficos
    return view('user-administrador.graficasbi', compact('charts'));
}
