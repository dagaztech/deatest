...
/*
    ...
    public function tablerosuseradmin(Request $request) {
        // Formulario 33: Datos de coordenadas (para el gráfico de calor)
        $coordenadas = FormValue::where("form_id", 33)
            ->get()
            ->map(function($item) {
                $json = json_decode($item->json, true);
                $coordLong = collect($json[0])->firstWhere('label', 'id coord-long')['value'];
                $coordLat = collect($json[0])->firstWhere('label', 'id coord-lat')['value'];
                return ['longitud' => $coordLong, 'latitud' => $coordLat];
            });
    
        // Formulario 10: Crecimiento de usuarios (para gráfico de crecimiento)
        $registroPorMes = FormValue::where("form_id", 10)
            ->select(DB::raw("DATE_FORMAT(created_at, '%Y-%m') as mes_anio, COUNT(id) as cantidad"))
            ->groupBy('mes_anio')
            ->orderBy('mes_anio')
            ->get();
    
        // Formulario de género (para gráfico de torta)
        $generos = FormValue::where("form_id", 11)
            ->get()
            ->map(function($item) {
                $json = json_decode($item->json, true);
                $genero = collect($json[0])->firstWhere('label', 'id select-1703782176523-0')['value'];
                return $genero;
            })
            ->countBy(); // Contamos cuántos de cada género
    
        return view('user-administrador/tableros-useradmin', compact('coordenadas', 'registroPorMes', 'generos'));
    }
