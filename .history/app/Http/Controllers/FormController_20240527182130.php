...
@header('Content-type: text/html; charset=utf-8');
    ...
    public function informeubicaciondeas(){
        ...
        foreach($datos as $dato => $item) { //foreach element in $arr
            $arr = json_decode($item["json"],true);
            $deasInstalacion = array_filter($arr[1], function($campo) {
              if($campo['label'] == "DEA's disponibles en el lugar"){
                return $campo;
              }
              return null;
            });
            $latitud = array_filter($arr[2], function($campo) {
            ...
            $tempDatos = strtoupper($deasInstalacion[1]["value"]) . " - " . $latitud[2]["value"] . " - " . $longitud[3]["value"];
