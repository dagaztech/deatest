...
class PagesController extends Controller
{
    ...
    public function mostrarTablero(Request $request)
{
    ...
    $lista = Registro::all();
    $lista2 = DEAInstalacion::all();
    $lista3 = EdadPromedio::all();

    if ($filtro === 'mes') {
        $lista = $lista->where('mes', substr($fecha, 5, 2));
        $lista2 = $lista2->where('mes', substr($fecha, 5, 2));
    } elseif ($filtro === 'dia') {
        $lista = $lista->where('fecha', $fecha);
        $lista2 = $lista2->where('fecha', $fecha);
    } elseif ($filtro === 'rango') {
        $fechas = explode(' to ', $rangoFechas);
        $lista = $lista->whereBetween('fecha', [$fechas[0], $fechas[1]]);
        $lista2 = $lista2->whereBetween('fecha', [$fechas[0], $fechas[1]]);
    }
