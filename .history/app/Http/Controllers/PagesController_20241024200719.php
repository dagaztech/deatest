    ...
    public function tablerosuseradmin(Request $request) {
        ...
        $coordenadas = FormValue::where("form_id", 33)
            ...
            ->map(function($item) {
                ...
                $coordLong = collect($json[0])->firstWhere('id', 'coord-long')['value'];
                $coordLat = collect($json[0])->firstWhere('id', 'coord-lat')['value'];
        ...
        $generos = FormValue::where("form_id", 14)
            ...
            ->map(function($item) {
                $json = json_decode($item->json, true);
                $genero = collect($json[0])->firstWhere('name', 'select-1703782176523-0')['value'];
                return $genero;
            })
