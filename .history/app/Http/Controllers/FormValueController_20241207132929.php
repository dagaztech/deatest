...
/*
    ...
    public function generateCharts()
{
    // Consultar todos los formularios desde la tabla tbl_forms y ordenarlos por su ID
    ...
    // Definir el mapa de equivalencias ID - Nombre
    $formNames = [
    ...
    // Iterar sobre cada formulario para generar su gráfico
    foreach ($forms as $form) {
        // Obtener el nombre del formulario según el ID o usar "Formulario Desconocido" si no está en el mapa
        ...
        $charts[] = [
            'form_id' => $form->id, // ID del formulario (opcional si lo necesitas)
...
{
    $forms = Form::all()->sortBy('id'); // Ordenamos los formularios de menor a mayor por ID
    ...
    $sheets = $forms->map(function ($form) {
        ...
        // Devuelve una instancia de ChartDataExportWithGraph para cada formulario
        return new ChartDataExportWithGraph($form->name, $data, [
            'type' => 'line', // Tipo de gráfico
            'labels' => array_column($data, 0), // Meses
            'data' => array_column($data, 1),   // Registros
        ]);
    })->toArray(); // Convertimos la colección a un arreglo

    // Ahora la función debería retornar un archivo Excel con múltiples hojas
    ...
    public function downloadGlobalCharts()
{
    ...
    // Crear hojas dinámicas para cada formulario con gráficos y datos
    $sheets = $forms->map(function ($form) use ($formNames) {
        // Obtener el nombre del formulario o usar "Formulario Desconocido"
        $formName = $formNames[$form->id] ?? "Formulario Desconocido";

        // Generar datos simulados (puedes reemplazar con datos reales de la BD)
        $data = [
            ['Mes', 'Registros'],
            ['Enero', rand(10, 100)],
            ['Febrero', rand(10, 100)],
        ...
        // Generar el gráfico para los datos
        $chart = [
        ...
        // Retornar un exportador personalizado para el formulario con su nombre
        return new ChartDataExportWithGraph($formName, $data, $chart);
    });

    // Retornar el archivo Excel con todas las hojas
    return Excel::download(new MultiSheetExport($sheets), 'todos_los_graficosbi.xlsx');
}
