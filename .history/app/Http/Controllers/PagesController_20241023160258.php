...
class PagesController extends Controller
{
    ...
    /*public function tablerosuseradmin(Request $request){
        ...
        $respuestas = FormValue::where("form_id", 14)->get();
        if(isset($request->busqueda)){
            $respuestas = FormValue::where("form_id", 14)->where(strtolower('tbl_form_values.json'), 'iLIKE', '%' . strtolower($request->busqueda) . '%')->get();
        }
        ...
        if (isset($request->rango)) {
            ...
            $start = $rango[0];
            $end = $rango[1];
            $respuestas = FormValue::where("form_id", 14)->whereBetween('created_at', [Carbon::parse($start), Carbon::parse($end)])->get();
        }
        ...
        $respuestas2 = FormValue::where("form_id", 11)->get();
        if(isset($request->busqueda)){
            $respuestas2 = FormValue::where("form_id", 11)->where(strtolower('tbl_form_values.json'), 'iLIKE', '%' . strtolower($request->busqueda) . '%')->get();
        }
        ...
        if (isset($request->rango)) {
            ...
            $start = $rango[0];
            $end = $rango[1];
            $respuestas2 = FormValue::where("form_id", 11)->whereBetween('created_at', [Carbon::parse($start), Carbon::parse($end)])->get();
        }


        $lista2 = array();

        foreach ($respuestas2 as $key => $value) {
            //print_r($value->updated_at);
            ...
            $nombre = array_filter($arr[0], function($campo) {
              ...
              if($campo['label'] == "Cantidad de DEAs"){
                //echo $campo['value'];
                return $campo;
    ...
    public function tablerosuseradmin(Request $request){
        ...
        if (isset($request->busqueda)) {
            $respuestas = FormValue::where("form_id", 14)
                ->where(strtolower('tbl_form_values.json'), 'iLIKE', '%' . strtolower($request->busqueda) . '%')
                ->get();
        }
        ...
        if (isset($request->rango)) {
            ...
            // Verificar si es un rango o una única fecha
            if (count($rango) == 2) {
                $start = $rango[0];
                $end = $rango[1];
                $respuestas = FormValue::where("form_id", 14)
                ...
                // Si es una única fecha
                $fecha = $rango[0];
                $respuestas = FormValue::where("form_id", 14)
                    ->whereDate('created_at', Carbon::parse($fecha))
                    ->get();
            }
        }
