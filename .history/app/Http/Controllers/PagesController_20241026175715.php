...
/*
    ...
    public function tablerosuseradmin(Request $request)
    {
        // Filtrar por rangos de fechas o día específico
        $respuestas = FormValue::where("form_id", 14);
        ...
        if (isset($request->dia)) {
            $respuestas = $respuestas->whereDay('created_at', $request->dia);
        }
        $respuestas = $respuestas->get();
    
        // Crear la lista de datos para el gráfico de registros por mes y día
        $lista = [];
        foreach ($respuestas as $value) {
            array_push($lista, [
                'mes' => date("m", strtotime($value->updated_at)),
                'dia' => date("d", strtotime($value->updated_at)),
            ]);
        }
    
        // Crear la lista de edades
        $lista3 = [];
        foreach ($respuestas as $value) {
            $arr = json_decode($value->json, true);
        ...
        // Crear la lista de cantidades de DEA por mes
        $respuestas2 = FormValue::where("form_id", 11);
        if (isset($request->rango)) {
            $respuestas2 = $respuestas2->whereBetween('created_at', [Carbon::parse($start), Carbon::parse($end)]);
        }
        if (isset($request->dia)) {
            $respuestas2 = $respuestas2->whereDay('created_at', $request->dia);
        }
        $respuestas2 = $respuestas2->get();
        ...
        foreach ($respuestas2 as $value) {
            $arr = json_decode($value->json, true);
            $cantidadDEA = collect($arr[0])->firstWhere('label', 'Cantidad de DEAs');
            if ($cantidadDEA) {
                array_push($lista2, [
                    'mes' => date("m", strtotime($value->updated_at)),
                    "cantidad" => $cantidadDEA["value"]
                ]);
            }
        }
    
        // Retornar vista con datos compactados
        return view('user-administrador/tableros-useradmin', compact('lista', 'lista2', 'lista3'));
    }
