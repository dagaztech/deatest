...
class FormValueController extends Controller
{
    ...
    public function extraerDatosAutomaticos($form_id)
{
    try {
        // Hacer una solicitud interna a la ruta que genera la vista
        $response = Http::get(url("/form-values/$form_id/view"));
        ...
        if ($response->successful()) {
            ...
            \Log::info('HTML recibido: ' . $html);

            // Procesar el HTML para extraer los datos
            $datos = $this->procesarHTMLTabla($html);

            if (isset($datos['error'])) {
                return response()->json(['error' => $datos['error']], 500);
            }

            return response()->json(['datos' => $datos]);
        }

        return response()->json(['error' => 'Error al cargar la vista: ' . $response->status()], 500);
    } catch (\Exception $e) {
        return response()->json(['error' => 'Excepción: ' . $e->getMessage()], 500);
    }
}

    private function procesarHTMLTabla($html)
    {
        try {
            $dom = new \DomDocument();
            ...
            // Intentar encontrar cualquier tabla en el HTML
            $tabla = $xpath->query('//table'); // Busca todas las tablas
    
            if ($tabla->length === 0) {
                return ['error' => 'No se encontró ninguna tabla en el HTML.'];
            }
