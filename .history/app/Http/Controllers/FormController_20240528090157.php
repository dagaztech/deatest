...
class FormController extends Controller 
{
    ...
    public function sincronizarlistarepresentantes($id)
    {
     ...
     public function sincronizartitulosplanes($id)
     {
         ...
         foreach($respuestas as $i => $item) { //foreach element in $arr
             ...
             $arr = json_decode($item["json"],true);
             $nombre = array_filter($arr[0], function($campo) {
               if($campo['label'] == "Título del plan"){
                 return $campo;
               }
               return null;
             });
         ...
         foreach($arr[0] as $i => $item) { //foreach element in $arr
             ...
             if($item["label"] == "Título del plan de mejora registrado"){
                 $item["values"] = array();
    ...
    public function sincronizarlistadeas($id)
    {
        ...
        foreach($respuestas as $i => $item) { //foreach element in $arr
            ...
            $serial = array_filter($arr[1], function($campo) {
              if($campo['label'] == "NÃºmero de serie"){
                return $campo;
              }
              return null;
            });
    ...
    public function sincronizarlistadeas4($id)
    {
        ...
        public function sincronizarlistadeas6($id)
        {
            ...
            $arr = json_decode($form->json,true);
            foreach($arr[0] as $i => $item) { //foreach element in $arr
                if($item["label"] == "DEA en revisiÃ³n"){
                    $item["values"] = array();
                    foreach($nombres as $nom) {
                        array_push($item["values"], 
                            [
            ...
            foreach($arr[0] as $i => $item) { //foreach element in $arr
                ...
                if($item["label"] == "Título del plan de mejora registrado"){
                    ...
                    array_push($item["values"], 
                        [
                        'label' => strtoupper($request->{"namefield"}),
                        'value' => strtoupper($request->{"namefield"}),
                        'selected' => false
                        ]);
                    ...
                    if ($row->type == 'checkbox-group') {
                        ...
                        if ($row->subtype == "fineuploader") {
                            ...
                            if ($fileSize < $fileLimit) {
                                    ...
                                    if ($request->hasFile($row->name)) {
                                        ...
                                        foreach ($files as $file) {
                                            ...
                                            if ($check) {
                                                ...
                                                if (isset($request->ajax)) {
                                                    return response()->json(['is_success' => false, 'message' => __("Tipo de archivo no válido. Cargue únicamente archivos con extensiones $requiredextention")], 200);
                                                } else {
                                                    return redirect()->back()->with('failed', __("Tipo de archivo no válido. Cargue únicamente archivos con extensiones $requiredextention"));
                                                }
                                            }
                                        }
                                        $row->value = $values;
                                ...
                                return response()->json(['is_success' => false, 'message' => __("Por favor cargue archivos con máximo $row->max_file_size_mb MB.")], 200);
                            }
                        }
...
*/
    ...
    public function buscarUsoDesfibrilador($documentoIdentidadPaciente)
    {
        ...
        foreach($respuestaFormularioPaciente as $i => $item) { //foreach element in $arr
            ...
            $documento = array_filter($arr[0], function($campo) {
              if($campo['label'] == "NÃºmero de documento de identidad"){
                return $campo;
              }
              return null;
            });
    ...
    public function buscarInformacionInstalacion($deaSeleccionado)
    {
        ...
        $direccion = array_filter($respuestasUinstalacion[0], function($campo) {
          if((str_contains(strtoupper($campo['label']), strtoupper("DirecciÃ³n del lugar")))){
    ...
    public function obtenerInformacionInformacionApis($formValueId, $request)
    {
        ...
        $telefonoObjeto = array_filter($arr[0], function($campo) {
            ...
            if($campo['label'] == "Número de teléfono"){
                return $campo;
            }
    ...
    public function obtenerInformacionInformacionApis1($formValueId)
    {
        ...
        if($idFormularioInformacionEvento == null){
            return "Recuerde que debe diligenciar el Formulario de INFORMACIÃ“N DEL EVENTO";
        }
        ...
        $documentoPacienteObjeto = array_filter($listaRespuetasInformacionEvento[0], function($campo) {
            // echo $campo['label'];
            // echo "<br />";
          if(str_contains(strtoupper($campo['label']), strtoupper("NÃºmero de documento de identificaciÃ³n del paciente"))){
            return $campo;
          }
          return null;
        });
        ...
        if($idFormUsoDesfribrilador == null){
            return "Recuerde que debe diligenciar el Formulario de reporte uso de Desfibrilador Externo AutomÃ¡tico - DEA en ambiente extrahospitalario";
        }
        ...
        $tipoDocumentoObjeto = array_filter($respuestasUsoDesfebrilador[0], function($campo) {
            // echo $campo['label'];
            // echo "<br />";
          if(str_contains(strtoupper($campo['label']), strtoupper("Tipo de documento de identificaciÃ³n"))){
            return $campo;
          }
          return null;
        });
        ...
        $telefonoObjeto = array_filter($respuestasUsoDesfebrilador[0], function($campo) {
            // echo $campo['label'];
            // echo "<br />";
          if(str_contains(strtoupper($campo['label']), strtoupper("TelÃ©fono"))){
            return $campo;
          }
          return null;
        });
        ...
        $nombrepersonaUsaDeaObjeto = array_filter($listaRespuetasInformacionEvento[0], function($campo) {
            // echo $campo['label'];
            // echo "<br />";
          if(str_contains(strtoupper($campo['label']), strtoupper("Nombre de la persona que utilizÃ³ el DEA"))){
            return $campo;
          }
          return null;
        });
        ...
        $documentoPersonaUsaDeaObjeto = array_filter($listaRespuetasInformacionEvento[0], function($campo) {
            // echo $campo['label'];
            // echo "<br />";
          if($campo['label'] == "NÃºmero de documento de identificaciÃ³n"){
            return $campo;
          }
          return null;
        });
        ...
        $horaActivacionCadenaObjeto = array_filter($listaRespuetasInformacionEvento[0], function($campo) {
          ...
          if($campo['label'] == "Hora de activaciÃ³n cadena de supervivencia"){
            return $campo;
          }
          return null;
        });
        ...
        $horaUtilizacionDeaObjeto = array_filter($listaRespuetasInformacionEvento[0], function($campo) {
          ...
          if($campo['label'] == "Hora De utilizaciÃ³n del DEA"){
            return $campo;
          }
          return null;
        });
        ...
        $horaTrasladoDeaObjeto = array_filter($listaRespuetasInformacionEvento[0], function($campo) {
          ...
          if($campo['label'] == "Hora De traslado de la persona atendida a la instituciÃ³n de salud"){
            return $campo;
          }
          return null;
        });
...
@header('Content-type: text/html; charset=utf-8');
    ...
    public function formStatus(Request $request, $id)
    {
        ...
        return response()->json(['is_success' => true, 'message' => __('Estado del formulario cambiado exitosamente.')]);
    }
    ...
    public function formpaymentIntegrationstore(Request $request, $id)
    {
        ...
        if ($request->payment_type == "paystack") {
            if ($request->currency_symbol != 'â‚¦' || $request->currency_name != 'NGN') {
                return redirect()->back()->with('failed', __('Currency not suppoted this payment getway. please enter NGN currency and â‚¦ symbol.'));
            }
        }
        if ($request->payment_type == "paytm") {
            if ($request->currency_symbol != 'â‚¹' || $request->currency_name != 'INR') {
                return redirect()->back()->with('failed', __('Currency not suppoted this payment getway. please enter INR currency and â‚¹ symbol.'));
            }
        }
    ...
    public function formIntegrationStore(Request $request, $id)
    {
        ...
        return redirect()->back()->with('success', __('Integración de formulario exitosa.'));
    }
  ...
  /*  public function informeubicaciondeas(){
        ...
        $informeubicaciondeas = FormValue::where("form_id", 12)->get();
        //echo $informeubicaciondeas->json;
        return "";
        $datos = array();
        foreach($datos as $dato => $item) { //foreach element in $arr
            $arr = json_decode($item["json"],true);
            $deasInstalacion = array_filter($arr[0], function($campo) {
              if($campo['label'] == "DEA's disponibles en el lugar"){
                return $campo;
              }
              return null;
            });
            $latitud = array_filter($arr[1], function($campo) {
              if($campo['label'] == "Coordenada: Latitud"){
                return $campo;
              }
              return null;
            });
            $longitud = array_filter($arr[3], function($campo) {
             if($campo['label'] == "Coordenada: Longitud"){
                  return $campo;
              }
             return null;
            });
            array_push($datos, ["label" => $datos, "value" => $item["id"]]);
        }
        //return response()->json(['message' => __("Alertas")], 200);
        return view('user-consultassm/informe-ubicaciondeas', compact('datos'));
        }
    ...
    public function informeubicaciondeas()
        {
            ...
            $formValues = FormValue::where("form_id", 12)
                            ->select("DEA's disponibles en el lugar", 'Coordenada: Latitud', 'Coordenada: Longitud')
                            ->get();
    
            // Pasar los datos a la vista
            return view('user-consultassm/informe-ubicaciondeas', ['formValues' => $formValues]);
        }
    ...
    public function showMap(Request $request){
          ...
          return view("/user-consultassm/informe-ubicaciondeas",['locations'=>$locations]);
          
        } */

        public function mostrarMapa()
        {
            $datos = FormValue::where("form_id", 12)
        ...
        public function puntosMapa($formValueId)
        {
            $datos = FormValue::where("form_id", 12)->get();
    
            $coordenadas = array();
            foreach($datos as $i => $item) { //foreach element in $arr
    
                $arr = json_decode($item["json"],true);
                $coordenada = array_filter($arr[0], function($campo) {
                  if($campo['label'] == "DEA's disponibles en el lugar"){
                    return $campo;
                  }
                  return null;
                });
                $coordenada = array_filter($arr[1], function($campo) {
                    if($campo['label'] == "Coordenada: Latitud"){
                      return $campo;
                    }
                    return null;
                }); 
                $coordenada = array_filter($arr[2], function($campo) {
                    if($campo['label'] == "Coordenada: Longitud"){
                      return $campo;
                    }
                    return null;
                ...
                foreach($coordenada as $j => $item2) {
    
@foreach($datos as $dato)
                    print("<tr>
                        <td>{{ $dato->id }}</td>
                        <td>{{ $dato['Coordenada: Latitud'] }}</td>
                        <td>{{ $dato['Coordenada: Longitud'] }}</td>
                    </tr>")
@endforeach
