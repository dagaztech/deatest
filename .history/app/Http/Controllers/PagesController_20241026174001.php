...
public function tablerosuseradmin(Request $request)
{
    ...
    $lista = $respuestas->map(function ($value) {
        return date("m", strtotime($value->updated_at));
    });

    $lista3 = $respuestas->map(function ($value) {
        $arr = json_decode($value->json, true);
        $edad = collect($arr[0])->firstWhere('label', 'Edad');
        return $edad ? $edad['value'] : null;
    })->filter();
    ...
    $lista2 = $respuestas2->map(function ($value) {
        $arr = json_decode($value->json, true);
