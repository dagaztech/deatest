...
class PagesController extends Controller
{
    ...
    public function tablerosuseradmin(Request $request) {
        // Obtener todas las respuestas del formulario 14
        $respuestas = FormValue::where("form_id", 14)->get();
    
        // Filtro por rango o fecha única (single)
        if (isset($request->rango) && !empty($request->rango)) {
        ...
        } elseif (isset($request->single) && !empty($request->single)) {
        ...
        // Procesar la lista de edades (lista3)
        $lista3 = [];
        foreach ($respuestas as $value) {
        ...
        // Obtener todas las respuestas del formulario 11
        $respuestas2 = FormValue::where("form_id", 11)->get();
    
        // Aplicar filtro de rango o fecha única en el formulario 11
        if (isset($request->rango) && !empty($request->rango)) {
            $rango = explode(" to ", $request->rango);
