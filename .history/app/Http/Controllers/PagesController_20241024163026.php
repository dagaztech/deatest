...
class PagesController extends Controller
{
    ...
    {
        // Obtener las respuestas del formulario 14
        $respuestas = FormValue::where("form_id", 14)->get();
        
        // Filtro por búsqueda en caso de estar presente
        if (isset($request->busqueda)) {
            $respuestas = FormValue::where("form_id", 14)
                ->where('json', 'iLIKE', '%' . strtolower($request->busqueda) . '%')
                ->get();
        }
        ...
        if (isset($request->rango)) {
            ...
            // Si el rango tiene dos fechas, se filtra por un intervalo
            if (count($rango) == 2) {
                $start = $rango[0];
                $end = $rango[1];
                $respuestas = FormValue::where("form_id", 14)
            ...
            // Si solo tiene una fecha, se filtra por esa fecha específica
        ...
        // Procesar los datos para obtener la edad de los registros
        $lista3 = array();
        foreach ($respuestas as $value) {
            ...
            // Verificar si existe el campo de edad y agregarlo a la lista
        ...
        return view('user-administrador/tableros-useradmin', compact('lista3'));
    }
