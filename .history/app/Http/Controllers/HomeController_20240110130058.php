...
use AuthorizesRequests;
        ...
        if (!file_exists(storage_path() . "/installed")) {
            ...
            if ($usr->rol == 'Administrador') {
                ...
                $form = Form::whereIn('id', function ($query) use ($roleId) {
                    $query->select('form_id')->from('tbl_user_forms')->where('role_id', $roleId);
                })->count();
                $submittedForm = FormValue::select(['tbl_form_values.*', 'forms.title'])
                ...
                $forms = Form::where(function ($query) use ($roleId, $userId) {
                    $query->whereIn('id', function ($query1) use ($roleId) {
                        $query1->select('form_id')->from('tbl_assign_forms_roles')->where('role_id', $roleId);
                    })->OrWhereIn('id', function ($query1) use ($userId) {
                        $query1->select('form_id')->from('tbl_assign_forms_users')->where('user_id', $userId);
                    })->OrWhere('assign_type', 'public');
                })->count();
                $poll = Poll::count();
            }
            ...
            return view('user-administrador/index', compact('user', 'form', 'submittedForm', 'widgets', 'poll', 'forms'));
        }
    }
    ...
    public function indexDashboard(DashboardWidgetDataTable $dataTable)
    {
        ...
        if (\Auth::guard('api')->can('manage-dashboardwidget')) {
