...
{
    // Obtener los datos (según lo necesites)
    $data = Form::where('id', $request->form_id)
                    ->whereBetween('created_at', [$request->start_date, $request->end_date])
                    ->get();

    // Un segundo parámetro adicional, por ejemplo, podría ser el ID del formulario
    $additionalParam = $request->form_id;

    // Pasar ambos parámetros al exportar
    return Excel::download(new ChartDataExportWithGraph($data, $additionalParam), 'chart_data_with_graph.xlsx');
}
