...
public function filterChartData(Request $request, $formId)
{
    ...
    // Lógica para filtrar los datos según el rango de fechas
    $query = FormData::where('form_id', $formId);
    if ($startDate && $endDate) {
        $query->whereBetween('created_at', [$startDate, $endDate]);
    }
    $filteredData = $query->get();

    // Procesar los datos para enviarlos al gráfico
    $labels = $filteredData->pluck('month')->toArray();
    $data = $filteredData->pluck('count')->toArray();
