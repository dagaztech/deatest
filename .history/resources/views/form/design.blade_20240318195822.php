...
@section('title', __('Diseñar Formulario'))
@section('breadcrumb')
    <div class="col-md-12">
        <div class="page-header-title">
            <h4 class="m-b-10">{{ __('Diseñar Formulario') }}</h4>
        </div>
        <ul class="breadcrumb">
            ...
            <li class="breadcrumb-item active"> {{ __('Diseñar Formulario') }} </li>
        </ul>
    </div>
@endsection
@section('content')
        ...
        <div class="main-content">
            @if (isset($_SERVER['REQUEST_SCHEME']) && $_SERVER['REQUEST_SCHEME'] == 'http')
                <div class="alert alert-warning">
                    <b>
                        {{ __('Tenga en cuenta que las funciones de grabación de video y selfies solo están disponibles en sitios web HTTPS y no funcionan en sitios HTTP.') }}</b>
                </div>
            @endif
...
@if (\Auth::guard('api')->user()->rol = 'Operativo SSM')
@push('style')
<style>
.ui-sortable .frm-holder input[type=number]{
    pointer-events: none;
    opacity: 0.5;
    cursor:default;
}
