...
<script>
 document.querySelector('input[name="cedulacode"]').onkeydown = function () {
...
<script>
 document.querySelector('input[name="cedulacodepaciente"]').onkeydown = function () {
...
<script>
    document.querySelector('input[name="cedulacodedea"]').onkeydown = function () {
        if (this.value.length >=) {
        this.value = this.value.slice(0,12); 
    }else{
        alert="Ingrese entre 6 y 12 dígitos en su número de identificación"
...
document.querySelectorAll('textarea').forEach(function(textarea) {
    textarea.onblur = function () {
        if (this.value.length > 1200) {
            this.value = this.value.slice(0, 1200); // Limita el contenido a 1200 caracteres
        }

        if (this.value.length >= 1200) {
            alert("Ingrese máximo 1200 caracteres");
        }
    };
});
</script>
...
document.querySelector('input[type="file"]').onchange = function () {
    ...
    const nextBtn = document.getElementById('nextBtn'); // Botón de submit

    // Verifica si el campo de archivo tiene el atributo "required"
    if (this.hasAttribute('required')) {
        if (file) {
            ...
            if (!validImageTypes.includes(fileType)) {
                alert("Por favor, seleccione una imagen válida (jpg, jpeg, png o gif).");
                this.value = ''; // Reinicia el campo si el archivo no es válido
                nextBtn.disabled = true; // Desactiva el botón si no es válido
            } else {
                alert("Imagen válida seleccionada.");
                this.setAttribute('data-completado', 'true'); // Marca el input como completado
                nextBtn.disabled = false; // Habilita el botón de submit
            }
        } else {
            alert("No se ha seleccionado ningún archivo.");
            nextBtn.disabled = true; // Desactiva el botón si no se selecciona nada
        }
    }
};
</script>
...
<script>
    ...
    function validateForm() {
        ...
        $('input[required], select[required], textarea[required], number[required], file[required], date[required], input[type=date], input[type=time], input[type=datetime-local], input[type=month], password[required]').each(function(){
            if ($(this).val() === '') {
                allFilled = false;
                return false; 
            }
        ...
        var maxLength12 = 12;
        $('#dea-usuario-contrasena').each(function(){
        ...
        if (allFilled && idsValid) {
            $('#nextBtn').prop('disabled', false);
        } else {
            $('#nextBtn').prop('disabled', true);
        }

        if (window.location.href == 'https://dea.wearesmart.co/forms/survey/wX90Gz3YaOpbxqlW142B') {
    ...
    $('input[required], select[required], textarea[required], number[required], file[required], date[required], input[type=date], input[type=time], input[type=datetime-local], input[type=month], password[required], #cedulacode, #cedulacodedea, #cedulacodepaciente, #dea-usuario-contrasena').on('keydown', function() {
        validateForm();
    });
...
<script>
  ...
  <script>
  if (window.location.href == 'https://dea.wearesmart.co/forms/survey/PLG6vkQBeXrkbyK82gAj') {
    $(document).ready(function() {
        // Ocultar ambos divs al cargar la página usando data-name
        // Ocultar solo el tercer 'namefield', el primero debe estar visible
        $('[data-name="namefield"]').eq(3).hide(); // Ocultamos el tercero (índice 2)
        $('[data-name="text-1703769659485-0"]').hide();

        // Manejar el cambio de selección usando el atributo name del select
        $('select[name="select-1703769572024-0"]').change(function() {
            var selectedValue = $(this).val();

            // Ocultar ambos divs antes de mostrar el correspondiente
            $('[data-name="namefield"]').eq(3).hide(); // Ocultamos el tercero nuevamente
            $('[data-name="text-1703769659485-0"]').hide();
            ...
            if (selectedValue === 'otro') {
                ...
                $('[data-name="namefield"]').eq(3).show(); // Mostrar solo el tercer 'namefield'
            }
        });
    });
}
    ...
    <!--RESTRINGIR FECHAS EM DATE-TIMES -->

    <script>
        document.addEventListener('DOMContentLoaded', function() {
          const camposFecha = [
            'text-1704420934171-0-',
            'text-1704420987306-0',
            'text-1704421057453-0',
            'text-1704421115044-0'
          ];
    
          // Función para verificar si el valor de una fecha es mayor que la anterior
          function verificarFechas() {
            let fechaAnterior = null;
    
            for (let i = 0; i < camposFecha.length; i++) {
              const campoActual = document.querySelector(`input[name="${camposFecha[i]}"]`);
    
              campoActual.addEventListener('change', function() {
                const valorActual = new Date(campoActual.value);
    
                // Si existe una fecha anterior, la comparamos con la actual
                if (fechaAnterior && valorActual < fechaAnterior) {
                  alert('La fecha no puede ser menor que la fecha anterior.');
                  campoActual.value = ''; // Limpiar el campo si la fecha es inválida
                } else {
                  fechaAnterior = valorActual;
                }
              });
            }
          }
    
          // Inicializar la verificación de fechas
          verificarFechas();
        });
</script>
<!-- PARA ACTIVAR BOTON NEXT SI SE SELECCIONO ALGUNA FECHA REQUIRED-->
<script>
    document.addEventListener('DOMContentLoaded', function() {
      // Seleccionamos todos los campos de tipo date, time, datetime-local, y month que sean required
      const camposFecha = document.querySelectorAll('input[type="date"][required], input[type="time"][required], input[type="datetime-local"][required], input[type="month"][required]');
      const nextBtn = document.getElementById('nextBtn');

      // Función que verifica si todos los campos requeridos tienen un valor
      function verificarCampos() {
        ...
        // Si todos los campos requeridos tienen un valor, se habilita el botón
        if (todosLlenos) {
      ...
      // Agregamos un evento 'change' a cada campo para verificar si se selecciona un valor
      camposFecha.forEach(function(campo) {
