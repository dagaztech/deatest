...
<script>
   document.addEventListener('DOMContentLoaded', function () {
    ...
    charts.forEach(chart => {
        ...
        // Filtrar por fecha a través de búsqueda de texto (ingreso manual)
        document.querySelector(`#search-${chart.form_id}`).addEventListener('input', function () {
            const inputText = this.value.trim();
            const validDates = parseDates(inputText); // Función para parsear la fecha ingresada
            const tablaId = `table-${chart.form_id}`;
            const filas = document.querySelectorAll(`#${tablaId} tbody tr`);
            let filteredData = [];
            let filteredLabels = [];
            ...
            if (validDates.length > 0) {
                filas.forEach(fila => {
                    ...
                    // Si la fecha coincide con alguno de los meses
                    if (validDates.includes(month)) {
                        fila.style.display = ''; // Mostrar fila
        ...
        function parseDates(inputText) {
            ...
            while ((match = regex.exec(inputText)) !== null) {
                ...
                if (!isNaN(fullDate)) {
                    ...
                    if (!validDates.includes(monthName)) {
                        validDates.push(monthName);
                    }
                }
            }
