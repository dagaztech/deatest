...
document.addEventListener('DOMContentLoaded', function () {
    ...
    charts.forEach(chart => {
        ...
        const startDateInput = document.getElementById(`fecha-inicio-${formId}`);
        const endDateInput = document.getElementById(`fecha-fin-${formId}`);

        const ctx = document.getElementById(chartId).getContext('2d');

        // Guardar datos y etiquetas originales
        ...
        function filterByDateRange() {
            ...
            originalLabels.forEach((label, index) => {
                const date = new Date(label); // Suponiendo que los labels son fechas
                if (
                    ...
                    filteredRows.push(index); // Guardar índice para filtrar la tabla
                }
            });
            ...
            tableRows.forEach((row, index) => {
                row.style.display = filteredRows.includes(index) ? '' : 'none';
            });
        }
...
<script>
    ...
    charts.forEach(chart => {
        ...
        const originalLabels = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'];
