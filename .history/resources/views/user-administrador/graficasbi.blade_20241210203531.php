    ...
    charts.forEach(chart => {
        ...
        function filterByDate() {
            ...
            if (startDate || endDate) {
                const mesesSeleccionados = obtenerMesesDesdeRango(startDate, endDate);

                filas.forEach(fila => {
                    const dateLabel = fila.querySelector('td').textContent;
                    const month = getMonthFromDate(dateLabel); // Obtener mes de la fecha

                    // Comprobar si el mes de la fila está dentro de los meses seleccionados
                    if (mesesSeleccionados.includes(month)) {
                        fila.style.display = '';
                ...
                // Actualizar gráfico con los meses filtrados
                chartInstance.data.labels = filteredLabels;
                chartInstance.data.datasets[0].data = filteredData;
                chartInstance.update();
            }
