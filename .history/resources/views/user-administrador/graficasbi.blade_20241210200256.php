...
@section('title', __('VisualizaciÃ³n de Tableros (GrÃ¡ficas BI)'))
        ...
        <div class="mx-auto col-md-12 rounded-card">
            <div class="card" id="purple-btn">
                <div class="card-header">
                    <h5 class="text-center w-100" id="new-title">VisualizaciÃ³n de Tableros (GrÃ¡ficas BI)</h5>
                </div>
                        ...
                        <div class="graphicblock">
                            ...
                            <div class="graphicblocker col-md-6 col-sm-12" data-form-id="{{ $chart['form_id'] }}">
                                ...
                                <div class="chart-controls">
                                    <label for="chartType-{{ $chart['form_id'] }}">Tipo de GrÃ¡fico:</label>
                                    <select id="chartType-{{ $chart['form_id'] }}" class="chart-type-selector">
                                        <option value="bar" {{ $chart['type'] == 'bar' ? 'selected' : '' }}>Barra</option>
                                        <option value="line" {{ $chart['type'] == 'line' ? 'selected' : '' }}>LÃ­nea</option>
                            ...
                            <div class="table-container col-md-5 col-sm-12">
                                ...
                                <p class="no-result" id="no-result-{{ $chart['form_id'] }}" style="display:none; text-align:center; color:red;">Resultado no encontrado. Intente con otro tÃ©rmino de bÃºsqueda.</p>
                            </div>
                            <p class="clear">&nbsp;</p>
                        @endforeach
        ...
        <div class="footer_gov_">
            <div class="centradototal_ fooflex">
                ...
                <div class="alcaldia_mod_footer">
                    <a href="https://www.medellin.gov.co/es"><img class="log_nww_footer" src="../../images/logo_nav_footer.png" alt="AlcaldÃ­a de MedellÃ­n"></a>
                </div>
            </div>
        </div>
    </div>
   ...
   document.addEventListener('DOMContentLoaded', function () {
    ...
    charts.forEach(chart => {
        ...
        const chartInstance = new Chart(ctx, {
            type: chart.type,  // Tipo de grÃ¡fico basado en el valor inicial del servidor
            data: {
                labels: chart.labels,
                datasets: [{
        ...
        document.querySelector(`#fecha-${chart.form_id}`).addEventListener('change', function () {
            ...
            // Actualizar grÃ¡fico con los datos filtrados
...
document.querySelectorAll('.download-excel').forEach(button => {
    ...
    button.addEventListener('click', function () {
        ...
        const yStart = yPos + imgHeight + 10; // Espaciado despuÃ©s del grÃ¡fico
        const columnHeaders = [];
        const rowData = [];
        ...
        // Dibujar tabla con lÃ­neas visibles
        doc.autoTable({
            startY: yStart,
            head: [columnHeaders],
            body: rowData,
            styles: { halign: 'center', valign: 'middle' },
            theme: 'grid', // Tema para incluir lÃ­neas
        });
    ...
    charts.forEach(chart => {
        ...
        document.querySelector(`#search-${chart.form_id}`).addEventListener('input', function () {
            ...
            filas.forEach(fila => {
                ...
                // Recorremos todas las celdas de la fila y comprobamos si alguna contiene el tÃ©rmino de bÃºsqueda
                columns.forEach(col => {
                ...
                if (matchFound || searchTerm === '') {
                    fila.style.display = ''; // Mostrar fila
                    filteredData.push(parseInt(columns[1].textContent)); // Asumimos que la segunda columna es la que contiene los nÃºmeros
...
document.querySelectorAll('.export-chart-png').forEach(button => {
    button.addEventListener('click', function () {
        ...
        if (canvas) {
            ...
            console.error(`No se encontrÃ³ el canvas con ID: chart-${formId}`);
        }
    });
});
</script>
