                                ...
                                <table class="table" id="table-{{ $chart['form_id'] }}">
                                    ...
                                    <tbody>
                                        @foreach($chart['data'] as $index => $value)
                                            <tr>
                                                <td>{{ $chart['random_dates'][$index] }}</td>
                                                <td>{{ $value }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
...
<script>
   document.addEventListener('DOMContentLoaded', function () {
    ...
    function generateRandomDate(startDate, endDate) {
        ...
        return randomDate.toISOString().split('T')[0];  // Retorna la fecha en formato 'YYYY-MM-DD'
    }

    charts.forEach(chart => {
        // Generar un rango de fechas aleatorias para este gráfico
        let randomDates = [];
        for (let i = 0; i < chart.data.length; i++) {
            const startDate = "2024-04-14";  // Fecha de inicio (por ejemplo: 14 de abril)
            const endDate = "2024-05-28";    // Fecha de fin (por ejemplo: 28 de mayo)
            randomDates.push(generateRandomDate(startDate, endDate));
        }
        chart.random_dates = randomDates;  // Asignamos las fechas aleatorias generadas

        const ctx = document.getElementById('chart-' + chart.form_id).getContext('2d');

        // Crear gráfico inicial con las fechas aleatorias
        const chartInstance = new Chart(ctx, {
            type: chart.type,
            data: {
                labels: chart.random_dates,  // Usamos las fechas aleatorias como etiquetas
                datasets: [{
        ...
        document.querySelector(`#search-${chart.form_id}`).addEventListener('input', function () {
            ...
            filas.forEach(fila => {
                ...
                if (matchFound || searchTerm === '') {
                    ...
                    filteredDates.push(columns[0].textContent);
                } else {
