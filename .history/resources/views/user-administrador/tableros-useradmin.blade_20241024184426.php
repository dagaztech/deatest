...
<script>
  $(document).ready(function() {
      ...
      // Evento para el botón "Mostrar información"
      $('#mostrarResultados').on('click', function(e) { e.preventDefault(); // Evitar recargar la página 
        // Obtener el tipo de filtro seleccionado 
        var tipo = $('#selectorTipoFecha').val(); var fechaInicio = null; var fechaFin = null; if (tipo === 'dia') { fechaInicio = $('#inputFechaUnica').val(); console.log("Fecha seleccionada (día):", fechaInicio); } else if (tipo === 'rango') { var rango = $('#inputRangoFechas').val().split(' to '); fechaInicio = rango[0]; fechaFin = rango[1]; console.log("Rango de fechas:", fechaInicio, fechaFin); } 
        // Limpiar los gráficos anteriores 
        $("#graficaUso").html(''); $("#graficaInstalacion").html(''); $("#graficaEdad").html(''); 
