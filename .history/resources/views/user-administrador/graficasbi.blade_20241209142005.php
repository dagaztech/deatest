...
<input type="month" class="filter-date range-end" data-form-id="{{ $chart['form_id'] }}">
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function () {
    const charts = @json($charts); // Configuración inicial de los gráficos
    ...
    charts.forEach(chart => {
        ...
        const filterByMonth = (selectedMonth) => {
            // Convertir el mes seleccionado al formato numérico (0 = enero, 11 = diciembre)
            const selectedMonthIndex = new Date(`${selectedMonth}-01`).getMonth();

            // Filtrar datos del gráfico y etiquetas
            const filteredData = [];
            const filteredLabels = [];
            originalLabels.forEach((label, index) => {
                const currentDate = new Date(label); // Asegúrate de que el formato de la etiqueta sea reconocible como fecha
                if (currentDate.getMonth() === selectedMonthIndex) {
            ...
            tableRows.forEach(row => {
                const rowDate = new Date(row.getAttribute('data-date')); // Suponiendo que cada fila tiene un atributo data-date
                if (rowDate.getMonth() === selectedMonthIndex) {
        ...
        // Event Listener para el campo de mes
        const monthInput = document.getElementById(`mes-selector-${chart.form_id}`);
