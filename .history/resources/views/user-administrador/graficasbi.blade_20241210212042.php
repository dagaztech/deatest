            ...
            document.querySelector(`#search-${chart.form_id}`).addEventListener('input', function () {
                ...
                filas.forEach(fila => {
                    ...
                    if (matchFound || searchTerms.length === 0) {
                        fila.style.display = '';
                        filteredLabels.push(columns[0].textContent); // Mes
                        filteredData.push(parseInt(columns[1].textContent)); // Registros
                        found = true;
                    } else {
                ...
                // Mostrar u ocultar el mensaje de "Resultado no encontrado"
                const noResultMessage = document.getElementById(`no-result-${chart.form_id}`);
                if (found) {
