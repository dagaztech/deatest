              ...
              document.querySelector(`#search-${chart.form_id}`).addEventListener('input', function () {
                  ...
                  if (fechas.length === 2) {
                      ...
                      fechas.forEach(fecha => {
                          const regexFecha = /^(\d{2})([-\/\s]?)(\d{2})\2?(\d{2,4})?$/;
                          const matchFecha = fecha.match(regexFecha);
  
                          if (matchFecha) {
                              const dia = matchFecha[1];
                              const mes = matchFecha[3];
                              const anio = matchFecha[4] || new Date().getFullYear().toString().slice(-2);
                              rangoFechas.push(new Date(`20${anio}`, mes - 1, dia));
                          }
                      });
                      ...
                      if (rangoFechas.length === 2) {
                          ...
                          filas.forEach(fila => {
                              ...
                              const mesFecha = new Date(Date.parse(mesTexto + " 1, 2000")); // Crear fecha para comparar (asume año genérico)
