...
<script>document.addEventListener('DOMContentLoaded', function () {
    ...
    // Crear gráfico inicial y aplicar filtros
    charts.forEach(chart => {
        ...
        $(`#fecha-range-${chart.form_id}`).daterangepicker({
            ...
            autoUpdateInput: true, // Actualizar el input cuando se seleccione el rango
            maxDate: moment(), // Fecha máxima: hoy
            minDate: moment().startOf('year'), // Fecha mínima: 1 de enero del año actual
            startDate: moment().startOf('year'), // Rango por defecto: 1 de enero
            endDate: moment().endOf('year'), // Rango por defecto: 31 de diciembre
            showDropdowns: true, // Mostrar los dropdowns para elegir el año y mes
        });
        ...
        $(`#fecha-range-${chart.form_id}`).on('apply.daterangepicker', function (ev, picker) {
            ...
            filterData(chart.form_id, startDate, endDate);
        });

        // Filtrar datos por texto
        document.querySelector(`#search-${chart.form_id}`).addEventListener('input', function () {
            const searchTerm = this.value.toLowerCase();
            const startDate = $(`#fecha-range-${chart.form_id}`).data('daterangepicker').startDate.format('YYYY-MM-DD');
            const endDate = $(`#fecha-range-${chart.form_id}`).data('daterangepicker').endDate.format('YYYY-MM-DD');
            // Llamar a la función de filtrado con la fecha y el término de búsqueda
            filterData(chart.form_id, startDate, endDate, searchTerm);
        });
    });

    // Función para filtrar la tabla y el gráfico según el rango de fechas y el término de búsqueda
    function filterData(formId, startDate, endDate, searchTerm = '') {
        const tablaId = `table-${formId}`;
        const chartInstance = Chart.instances.find(chart => chart.canvas.id === `chart-${formId}`);
        const filas = document.querySelectorAll(`#${tablaId} tbody tr`);

        // Datos de la tabla filtrados y etiquetas para el gráfico
        let filteredData = [];
        let filteredLabels = [];
        let foundData = false;
        ...
        filas.forEach(fila => {
            ...
            const dateCell = columns[0].textContent; // La fecha está en la primera columna
            const date = moment(dateCell, 'YYYY-MM-DD');
            const matchFound = columns.some(col => col.textContent.toLowerCase().includes(searchTerm.toLowerCase()));

            // Filtrar por fecha y término de búsqueda
            if (date.isBetween(startDate, endDate, null, '[]') && (matchFound || searchTerm === '')) {
                fila.style.display = ''; // Mostrar la fila
                filteredData.push(parseInt(columns[1].textContent)); // Asumimos que la segunda columna contiene los datos
        ...
        // Si se encuentran datos, actualizar el gráfico
        if (foundData) {
            ...
            document.getElementById(`no-result-${formId}`).style.display = 'none'; // Ocultar mensaje "no encontrado"
        } else {
