...
<script>
    ...
    rows.forEach(row => {
        const dateCell = row.querySelector('td[data-date]'); // Columna con la fecha
    ...
    // Filtrar gráficos
    charts.forEach(chart => {
        const filteredData = chart.originalData.filter((dataPoint, index) => {
            const pointDate = new Date(chart.labels[index]);
            return pointDate >= startDate && pointDate <= endDate;
        });

        chart.data.labels = filteredData.map((_, index) => chart.labels[index]);
...
<script>
    document.getElementById('chart-type-select').addEventListener('change', (event) => {
        const newType = event.target.value; // Obtener el tipo seleccionado
        charts.forEach(chart => {
            chart.config.type = newType; // Cambiar el tipo de gráfica
            chart.update(); // Actualizar la gráfica
        });
    });
    </script>
    <script>
        // Inicializar la gráfica con datos iniciales
const ctx = document.getElementById('chart-canvas').getContext('2d');
let chart = new Chart(ctx, {
    type: 'bar', // Tipo inicial
    data: {
        labels: ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo' , 'Junio', 'Julio' , 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'], // Ejemplo de etiquetas
        datasets: [{
            label: 'Registros',
            data: [10, 20, 30, 40], // Ejemplo de datos
...
// Cambiar tipo de gráfica al seleccionar una nueva opción
document.getElementById('chart-type-select').addEventListener('change', (event) => {
    const newType = event.target.value; // Obtener el tipo de gráfica seleccionado

    // Destruir la gráfica anterior para evitar conflictos
    chart.destroy();

    // Crear una nueva gráfica con el tipo seleccionado
    chart = new Chart(ctx, {
        type: newType, // Tipo dinámico seleccionado
        data: {
            labels: ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo' , 'Junio', 'Julio' , 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'], // Usa los mismos datos originales
            datasets: [{
                label: 'Registros',
                data: [10, 20, 30, 40],
                backgroundColor: ['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0']
