        ...
        <div class="mx-auto col-md-12 rounded-card">
            <div class="card" id="purple-btn">
                ...
                <div class="card-body form-card-body">
                    ...
                    <div class="mx-auto mb-4 mt-0 row col-md-4">
                        <button id="download-global-pdf" class="btn btn-success">
                            Descargar Global de Datos en PDF
                        </button>
                    </div>
                        <hr>
                        ...
                        <div class="graphicblock">
                            ...
                            <h6>Formulario: {{ $chart['form_name'] }}                                 &nbsp; | &nbsp;
                                <a href="{{ route('download.chart.data', ['form_id' => $chart['form_id']]) }}" class="btn btn-sm btn-primary">Descargar Excel</a> | <button class="btn btn-sm btn-primary export-form-pdf" data-form-id="{{ $chart['form_id'] }}">
                                    Exportar PDF  </button>
                            </h6>
                            <form id="filterForm-{{ $chart['form_id'] }}" class="mb-4">
                                ...
                                <div class="table-container col-lg-5 col-md-5 col-sm-12">
                                    ...
                                    <button class="btn btn-sm btn-secondary export-table-png" data-form-id="{{ $chart['form_id'] }}">
                                        Exportar Tabla en PNG
                                    </button>
                                </div>
                                <p class="clear">&nbsp;</p>
                                <hr>
...
document.querySelectorAll('.export-table-png').forEach(button => {
    button.addEventListener('click', function () {
        const formId = button.getAttribute('data-form-id');
        const table = document.getElementById(`table-${formId}`);
        
        html2canvas(table).then((canvas) => {
            const imgData = canvas.toDataURL('image/png');
            const link = document.createElement('a');
            link.href = imgData;
            link.download = `tabla-${formId}.png`;
            link.click();
        });
    });
});
