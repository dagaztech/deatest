...
<script>
  $(document).ready(function() {
      ...
      function generarGraficos(filtroInicio = null, filtroFin = null) {
          ...
          // Si no hay datos filtrados, mantener los ejes vacíos pero no ocultar la gráfica
          var seriesUso = meses.map((mes, index) => dataFiltrada.filter(x => x == ("0" + (index + 1)).slice(-2)).length);
          var seriesInstalacion = meses.map((mes, index) => data2Filtrada.filter(x => x.mes == ("0" + (index + 1)).slice(-2)).reduce((sum, a) => sum + +a.cantidad, 0));
