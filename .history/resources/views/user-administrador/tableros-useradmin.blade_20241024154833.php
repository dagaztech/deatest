...
<input type="text" id="filtro-fecha" placeholder="Seleccionar fecha">

<!-- Selección de rango de fechas -->
<input type="text" id="filtro-rango" placeholder="Seleccionar rango de fechas" >
...
<script>
$(document).ready(function() {
    // Inicialización de Flatpickr para fecha única
    ...
    function aplicarFiltros() {
        var tipoFiltro = $("#filtro-tipo").val();
        var fechaSeleccionada = $("#filtro-fecha").val();
        var rangoSeleccionado = $("#filtro-rango").val();
        
        // Filtrar datos según tipo de filtro (día, mes, año) y fechas seleccionadas
        var dataFiltrada = data; // Usa `data`, `data2`, o `data3` según el gráfico

        if (tipoFiltro === "dia") {
            // Lógica para filtrar por día específico
            if (fechaSeleccionada) {
                dataFiltrada = data.filter(item => item.fecha === fechaSeleccionada);
            }
        } else if (tipoFiltro === "mes") {
            // Lógica para filtrar por mes
            if (fechaSeleccionada) {
                var mesSeleccionado = fechaSeleccionada.split("-")[1]; // Extraer mes
                dataFiltrada = data.filter(item => item.mes === mesSeleccionado);
            }
        } else if (tipoFiltro === "anio") {
            // Lógica para filtrar por año
            if (fechaSeleccionada) {
                var anioSeleccionado = fechaSeleccionada.split("-")[0]; // Extraer año
                dataFiltrada = data.filter(item => item.anio === anioSeleccionado);
            }
        }

        // Filtrar por rango de fechas si es necesario
        if (rangoSeleccionado) {
            var fechas = rangoSeleccionado.split(" to ");
            var inicio = fechas[0];
            var fin = fechas[1];

            dataFiltrada = data.filter(item => item.fecha >= inicio && item.fecha <= fin);
        }
    ...
    function actualizarGraficos(dataFiltrada) {
        // Actualizar gráfico de uso
        chart.updateSeries([{
            data: [
                dataFiltrada.filter(x => x.mes == "01").length,
                dataFiltrada.filter(x => x.mes == "02").length,
                dataFiltrada.filter(x => x.mes == "03").length,
                dataFiltrada.filter(x => x.mes == "04").length,
                dataFiltrada.filter(x => x.mes == "05").length,
                dataFiltrada.filter(x => x.mes == "06").length,
                dataFiltrada.filter(x => x.mes == "07").length,
                dataFiltrada.filter(x => x.mes == "08").length,
                dataFiltrada.filter(x => x.mes == "09").length,
                dataFiltrada.filter(x => x.mes == "10").length,
                dataFiltrada.filter(x => x.mes == "11").length,
                dataFiltrada.filter(x => x.mes == "12").length,
            ]
        }]);

        // Actualizar gráfico de instalaciones
        chart2.updateSeries([{
            data: [
                dataFiltrada.filter(x => x.mes == "01").reduce((partialSum, a) => partialSum + +a.cantidad, 0),
                dataFiltrada.filter(x => x.mes == "02").reduce((partialSum, a) => partialSum + +a.cantidad, 0),
                dataFiltrada.filter(x => x.mes == "03").reduce((partialSum, a) => partialSum + +a.cantidad, 0),
                dataFiltrada.filter(x => x.mes == "04").reduce((partialSum, a) => partialSum + +a.cantidad, 0),
                dataFiltrada.filter(x => x.mes == "05").reduce((partialSum, a) => partialSum + +a.cantidad, 0),
                dataFiltrada.filter(x => x.mes == "06").reduce((partialSum, a) => partialSum + +a.cantidad, 0),
                dataFiltrada.filter(x => x.mes == "07").reduce((partialSum, a) => partialSum + +a.cantidad, 0),
                dataFiltrada.filter(x => x.mes == "08").reduce((partialSum, a) => partialSum + +a.cantidad, 0),
                dataFiltrada.filter(x => x.mes == "09").reduce((partialSum, a) => partialSum + +a.cantidad, 0),
                dataFiltrada.filter(x => x.mes == "10").reduce((partialSum, a) => partialSum + +a.cantidad, 0),
                dataFiltrada.filter(x => x.mes == "11").reduce((partialSum, a) => partialSum + +a.cantidad, 0),
                dataFiltrada.filter(x => x.mes == "12").reduce((partialSum, a) => partialSum + +a.cantidad, 0),
            ]
        }]);
        
        // Actualizar gráfico de edades
        chart3.updateSeries([{
            data: Object.values(dataFiltrada.reduce((agg, curr) => {
                if (agg[curr.edad]) {
                    agg[curr.edad] += 1;
                } else {
                    agg[curr.edad] = 1;
    ...
    $("#filtro-tipo, #filtro-fecha, #filtro-rango").change(function() {
        aplicarFiltros();
    });
