    ...
    if (startDate && endDate) {
      ...
      row.style.display = ''; // Show all rows if no dates are selected
        ...
        function filterByDateRange() {
            ...
            filas.forEach(fila => {
                const fecha = fila.children[0].textContent; // Asumimos que la fecha está en la primera columna
                const isInRange = (fecha >= fechaInicio && fecha <= fechaFin);

                if ((isInRange || (fechaInicio === '' && fechaFin === ''))) {
                    fila.style.display = ''; // Mostrar fila
                    const dataValue = parseInt(fila.children[1].textContent); // El valor numérico está en la segunda columna
                    filteredData.push(dataValue);
                    filteredLabels.push(fila.children[0].textContent); // Fecha para las etiquetas
                } else {
        ...
        //document.querySelector(`#fecha-inicio-${chart.form_id}`).addEventListener('change', filterByDateRange);
        //document.querySelector(`#fecha-fin-${chart.form_id}`).addEventListener('change', filterByDateRange);
        document.getElementById(`fecha-inicio-${formId}`).addEventListener('change', () => filterByDateRange(formId));
        document.getElementById(`fecha-fin-${formId}`).addEventListener('change', () => filterByDateRange(formId));
    });
