        ...
        signaturePad.forEach(function(val) {
            ...
            saveButton.addEventListener('click', function(event) {
                ...
                $('#' + val).parent().find('.sign-error').html(
                    '<small class="text-success">Panel de firma guardado.</small>');
            ...
            clearButton.addEventListener('click', function(event) {
                ...
                $('#save' + val).attr('disabled', true);
                $('#' + val).parent().find('.sign-error').html(
                    '<small class="text-danger">Panel de firma limpiado.</small>');
                signaturePad.clear();
            });
            $(document).on('click', '#' + val, function() {
                ...
                $('#' + val).parent().find('.sign-error').html(
                    '<small class="text-danger">Nota: por favor guarde su panel de firma.</small>'); 
            });
        });
    </script>
<?php $__env->stopPush(); ?>
<?php /**PATH C:\xampp2\htdocs\dea-template-pwa-last\resources\views/form/js/signature.blade.php ENDPATH**/ ?>