...
class ProfileController extends Controller
{
    ...
    public function index()
    {
        if (!setting('2fa')) {
            ...
            $role = DB::table('tbl_roles')->where('id', 1)->first();
    ...
    public function BasicInfoUpdate(Request $request)
    {
        ...
        $log->crear($user->id, "Actualización datos Perfil", join(",", $request->ips()));
        $log->save();
    ...
    public function LoginDetails(Request $request)
    {
        $user = Auth::guard('api')->user();
        request()->validate([
            'email'                 => 'required|string|email|max:191|unique:tbl_users,email,' . $user->id,
        ...
        \Mail::mailer('smtp')->to($to)
            ->send(new RestauracionContrasena("",$request->password));


        $log = new HistoricoLog();
        $log->crear($user->id, "Cambio contraseña", join(",", $request->ips()));
        $log->save();
