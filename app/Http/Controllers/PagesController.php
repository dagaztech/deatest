...
class PagesController extends Controller
{
    ...
    {
        // Obtener todas las respuestas sin filtro
        $respuestas = FormValue::where("form_id", 14);
        $respuestas2 = FormValue::where("form_id", 11);
    
        // Filtrado por una fecha específica
        if ($request->filled('inputFechaUnica')) {
            $fecha = Carbon::parse($request->inputFechaUnica); // Ajuste para capturar correctamente la fecha del input
            $respuestas->whereDate('created_at', $fecha);
            $respuestas2->whereDate('created_at', $fecha);
        }
    
        // Filtrado por un rango de fechas
        if ($request->filled('inputRangoFechas')) {
            $rango = explode(" a ", $request->inputRangoFechas);
            if (count($rango) == 2) {
        ...
        // Construir listas para los gráficos
        $lista = array();
        ...
        // Agregar conteo de filas donde la columna 'No' coincide con la misma fecha
        $countItemsByDate = function ($query) use ($request) {
            if ($request->filled('inputFechaUnica')) {
                $fecha = Carbon::parse($request->inputFechaUnica);
                return $query->whereDate('created_at', $fecha)->count();
            } elseif ($request->filled('inputRangoFechas')) {
                $rango = explode(" a ", $request->inputRangoFechas);
                if (count($rango) == 2) {
                    $start = Carbon::parse($rango[0]);
                    $end = Carbon::parse($rango[1]);
                    return $query->whereBetween('created_at', [$start->startOfDay(), $end->endOfDay()])->count();
                }
            }
            return $query->count();
        };
    
        // Llamar la función de conteo para ambos conjuntos de respuestas
        $totalCountRespuestas = $countItemsByDate($respuestas);
        $totalCountRespuestas2 = $countItemsByDate($respuestas2);
    
        // Devolver datos filtrados en formato JSON si es una solicitud AJAX
        if ($request->ajax()) {
            return response()->json([
                'lista' => $lista,
                'lista2' => $lista2,
                'lista3' => $lista3,
                'totalCountRespuestas' => $totalCountRespuestas,
                'totalCountRespuestas2' => $totalCountRespuestas2
            ]);
        }
    
        return view('user-administrador/tableros-useradmin', compact('lista', 'lista2', 'lista3', 'totalCountRespuestas', 'totalCountRespuestas2'));
    }
