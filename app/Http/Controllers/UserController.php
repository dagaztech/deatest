...
class UserController extends Controller
{
    ...
    {
        if (\Auth::guard('api')->user()->users_grid_view == 1) {
    ...
    {
        $role = DB::table('tbl_roles')->where('name', '!=', 'Administrador')->get();
        $roles = [];
    ...
    {
        \Auth::guard('api')->user()->leaveImpersonation($request->user());
        return redirect('/home');
    }
}
