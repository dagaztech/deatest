...
class FormValuesDataTable extends DataTable
{
    public function dataTable($query)
    {
        ...
        if ($labels != null) {
            ...
            $arr = array_merge(['action', 'user',  'created_at'], array_keys($labels));
        } else {
    ...
    public function query(FormValue $model, Request $request)
    {
        ...
        if (($usr->rol == 'Usuario operador 1' || $usr->rol == 'Usuario consulta E' || $usr->rol == 'Consulta SSM' )&& ($this->form_id == 33)) {
            $formValues =  $model->newQuery()
            ->select(['tbl_form_values.*', 'tbl_forms.title'])
            ->join('tbl_forms', 'tbl_forms.id', '=', 'tbl_form_values.form_id')
            // ->where('tbl_form_values.user_id', '=', $usr->id)
        ...
        if (($usr->rol == 'Usuario operador 1' || $usr->rol == 'Usuario consulta E'  || $usr->rol == 'Consulta SSM' )&& ($this->form_id == 14))
        {
            $id_lugar = $this->buscar_lugar_por_formulario_usuario();
                // echo $id_lugar;
                // return;
            ...
            if($id_lugar != null){
                ...
                foreach ($formValues->get() as $key => $formulario) {
                    ...
                    $formValue2 = FormValue::where("form_id", 13)->where("json", 'LIKE', '%'.'"value":"'.$numeroDocumento.'"}'.'%', )->first();
                        
                    if($formValue2 != null){

                        $arr = json_decode($formValue2->json,true);
    ...
    {
        // print_r($this->getColumns($this->labels()));
        // return "";
        ...
        // $columns[] = Column::make('user')->title(__('user'));
