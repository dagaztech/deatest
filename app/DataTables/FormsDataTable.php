...
class FormsDataTable extends DataTable
{
            ...
            return $model->newQuery()
                ->where(function ($query) use ($role_id, $user_id) {
                    $query->whereIn('id', function ($query1) use ($role_id) {
                        $query1->select('form_id')->from('tbl_assign_forms_roles')->where('role_id', $role_id);
                    })->OrWhereIn('id', function ($query1) use ($user_id) {
                        $query1->select('form_id')->from('tbl_assign_forms_users')->where('user_id', $user_id);
    ...
    public function html()
    {
        ...
        $canCreateForm = \Auth::guard('api')->user()->rol == 'Administrador';
        $canExportForm = \Auth::guard('api')->user()->rol == 'Administrador';
